...
const Hero = () => {
  ...
  const [heroImage, setHeroImage] = useState('/assets/1.png');

  useEffect(() => {
    getSettings()
      .then(s => { if (s?.hero_image) setHeroImage(s.hero_image); })
      .catch(() => {});
  }, []);

  useEffect(() => {
    const tl = gsap.timeline({ defaults: { ease: "power2.out" } });
    tl.fromTo(titleRef.current, { opacity: 0, y: -30 }, { opacity: 1, y: 0, duration: 1.2 })
      .fromTo(descRef.current, { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 1 }, "-=0.8")
      .fromTo(buttonsRef.current, { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 1 }, "-=0.7")
      .fromTo(imageRef.current, { opacity: 0, scale: 0.9 }, { opacity: 1, scale: 1, duration: 1.2 }, "-=1");
  }, []);
      ...
      <div className="container grid grid-cols-1 md:grid-cols-2 gap-8 min-h-[500px] md:min-h-[600px] px-4">
        ...
        <div className="flex flex-col justify-center gap-4 md:gap-8 text-center md:text-left pt-8 md:pt-24 pb-10">
          ...
          <div ref={buttonsRef} className="flex flex-col sm:flex-row gap-3 md:gap-4 items-center md:items-start md:justify-start justify-center">
            ...
            <a
              href="https://wa.me/22901160557623?text=Bonjour%20FOODIRECT,%20je%20souhaite%20commander"
              target="_blank"
